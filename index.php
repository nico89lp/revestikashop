...
<body>
    ...
    <div class="container">
        ...
        <div id="quotes" class="tab-content">
            ...
            <div class="quote-total" id="quoteTotal" style="display: none;">
                ...
                <button type="button" class="btn" onclick="generateQuote()">📋 Generar Presupuesto</button>
                <button type="button" class="btn btn-pdf" onclick="downloadQuotePDF()">📄 Descargar PDF</button>
            </div>

            <div id="generatedQuote" class="print-section" style="display: none;">
    ...
    <script>
        ...
        let currentCurrency = 'ARS'; // Por defecto en pesos
        let exchangeRates = {
            current: null,
            historical: {}
        };
        ...
        function showTab(tabName) {
            ...
            try {
                ...
                setTimeout(async () => {  // ⭐ Agregar async aquí
                    try {
                        if (tabName === 'dashboard') {
                            ...
                            await initializeTaxesTab();  // ✅ Ahora sí funciona
                        }
                    } catch (error) {
        ...
        async function updateDashboard() {
            try {
                ...
                // CALCULAR CON CONVERSIÓN SEGÚN CRITERIOS
                let totalIncome = 0;
                let salesCashIncome = 0;
                let salesDigitalIncome = 0;

                for (const transaction of filteredData.filter(t => t.type === 'income' && t.category !== 'Aportes de Capital')) {
                ...
                for (const transaction of filteredData.filter(t => t.type === 'expense')) {
                    ...
                    } else if (transaction.expenseType === 'variable') {
                        variableExpenses += convertedAmount;
                    }
                }

                // SALDOS (usan TC del día - REGLA 3)
                let cashIncome = 0;
                let digitalIncome = 0;
                let cashExpenses = 0;
                ...
                for (const investment of investments) {
                    ...
                    totalInvestmentAmount += convertedAmount;
                }
                ...
                const elements = {
                    ...
                    'totalExpenses': formatCurrencySync(totalExpenses),
                    'fixedExpenses': formatCurrencySync(fixedExpenses),
                    'variableExpenses': formatCurrencySync(variableExpenses),
                    'totalInvestmentDashboard': formatCurrencySync(totalInvestmentAmount),
                    'cashBalance': formatCurrencySync(cashBalance),
                    'digitalBalance': formatCurrencySync(digitalBalance),
                    'pendingIncome': formatCurrencySync(pendingIncome),
                    'pendingExpenses': formatCurrencySync(pendingExpenses)
                };
                ...
                console.log('✅ Dashboard actualizado con conversión a:', currentCurrency);
        ...
        async function initializeApp() {
            try {
                console.log('Inicializando aplicación con sistema de conversión...');

                // Inicializar sistema de moneda
                initializeCurrencySystem();

                // Cargar datos desde BD
                transactions = await loadFromDatabase('transactions') || [];
                products = await loadFromDatabase('products') || [];
                ...
                // Actualizar todas las pantallas con conversión
                await updateAllDisplaysWithCurrency();
                
                console.log('Aplicación inicializada correctamente con conversión');
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            try {
                ...
                // Configurar eventos y formularios
                setTodayDates();
                ...
                // Inicializar aplicación con datos
                initializeApp().then(() => {
        ...
        // ======== VARIABLES GLOBALES PARA IMPUESTOS Y GASTOS FIJOS ========
        let taxFixedItems = [];
